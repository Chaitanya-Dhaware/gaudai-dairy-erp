...
import { PlusCircle, Trash2, Calendar, Sparkles, AlertTriangle } from 'lucide-react';
          ...
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full border border-black/[0.08] shadow-2xl space-y-6">
            ...
            <p className="text-xs text-textSecondary leading-relaxed">
              {isMarathi 
                ...
                : `A matching expense entry for ₹${duplicateWarning.amount} on ${formatDate(duplicateWarning.date)} already exists. Save this duplicate anyway?`}
            </p>
...
function AnalyticsView({
  ...
  useEffect(() => {
    let active = true;
    generateBusinessInsights(expenses, isMarathi ? 'mr' : 'en')
      .then(insights => {
        if (active) {
